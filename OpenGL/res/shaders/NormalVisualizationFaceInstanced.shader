...
void main() {
	...
	vec4 norm = vec4(normalize(cross(a, b))* 2.5, 0.0) ;
	...
	vec4 arrowHead = center + (norm * 1.75);
	vec4 normOrth1 = vec4(normalize(cross(a, vec3(norm))) * 0.5, 0.0);
	vec4 normOrth2 = vec4(normalize(cross(vec3(normOrth1), vec3(norm))) * 0.5, 0.0);
