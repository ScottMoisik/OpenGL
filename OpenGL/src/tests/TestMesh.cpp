...
namespace Test {
	...
	void TestMesh::OnUpdate(float deltaTime) {
		...
		m_MeshBox->m_InstanceModelMatrices.push_back(glm::translate(glm::mat4(1.0f), glm::vec3(5.0f, -5.0f, 5.0f)));
	...
	void TestMesh::OnRender() {
		...
		m_Proj = glm::perspective(glm::radians(45.0f), m_AspectRatio, 0.1f, 100.0f);
		{
			...
			/* Set up shadow rendering: render depth of scene from light's perspective */
